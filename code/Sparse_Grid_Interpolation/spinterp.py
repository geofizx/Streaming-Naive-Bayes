...
def runInterp(d,z_k,grdin,grdout,indx,mi,type,intvl):
	...
	if type == 'cc' or type == 'CC':
		for i in range(0,num2):			#i=:num2  # Number of points to interpolate
			for j in range(0,num4):		#j=1:num4  # Number of grid nodes at current level
				wght2[j] = 1.0			# Iinitialize total linear basis integration weights
				for l in range(0,d): 	#l=1:d  # Number of dimensions for the interpolation
